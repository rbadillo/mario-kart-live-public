...
// Arrow Left = 50
// Arrow Right = 60
// Stop Left Joystick = 20
// Stop Right Joystick = 10

function getGamepadState(){
	var gamepad = navigator.getGamepads();

	if(gamepad[0]){
		var leftjoystick = gamepad[0].axes[0]
		var rightjoystick = gamepad[0].axes[3]

		console.log("Left Joystick: " +leftjoystick)
		console.log("Right Joystick: " +rightjoystick)

		// Stop Forward/Reverse
		if(rightjoystick>-0.10 && rightjoystick<0.10){
			console.log("Stop rightjoystick")
			socket.emit('rpi_car_control',"10");

		// Forward
		}else if(rightjoystick<=-0.10 && rightjoystick>-0.20){
			console.log("Forward 30")
			socket.emit('rpi_car_control',"30");
		}else if(rightjoystick<=-0.20 && rightjoystick>-0.30){
			console.log("Forward 31")
			socket.emit('rpi_car_control',"31");
		}else if(rightjoystick<=-0.30 && rightjoystick>-0.40){
			console.log("Forward 32")
			socket.emit('rpi_car_control',"32");
		}else if(rightjoystick<=-0.40 && rightjoystick>-0.50){
			console.log("Forward 33")
			socket.emit('rpi_car_control',"33");
		}else if(rightjoystick<=-0.50 && rightjoystick>-0.60){
			console.log("Forward 34")
			socket.emit('rpi_car_control',"34");
		}else if(rightjoystick<=-0.60 && rightjoystick>-0.70){
			console.log("Forward 35")
			socket.emit('rpi_car_control',"35");
		}else if(rightjoystick<=-0.70 && rightjoystick>-0.80){
			console.log("Forward 36")
			socket.emit('rpi_car_control',"36");
		}else if(rightjoystick<=-0.80 && rightjoystick>-0.90){
			console.log("Forward 37")
			socket.emit('rpi_car_control',"37");
		}else if(rightjoystick<=-0.90 && rightjoystick>=-1.00){
			console.log("Forward 38")
			socket.emit('rpi_car_control',"38");

		// Reverse
		}else if(rightjoystick>=0.10 && rightjoystick<0.20){
			console.log("Reverse 40")
			socket.emit('rpi_car_control',"40");
		}else if(rightjoystick>=0.20 && rightjoystick<0.30){
			console.log("Reverse 41")
			socket.emit('rpi_car_control',"41");
		}else if(rightjoystick>=0.30 && rightjoystick<0.40){
			console.log("Reverse 42")
			socket.emit('rpi_car_control',"42");
		}else if(rightjoystick>=0.40 && rightjoystick<0.50){
			console.log("Reverse 43")
			socket.emit('rpi_car_control',"43");
		}else if(rightjoystick>=0.50 && rightjoystick<0.60){
			console.log("Reverse 44")
			socket.emit('rpi_car_control',"44");
		}else if(rightjoystick>=0.60 && rightjoystick<0.70){
			console.log("Reverse 45")
			socket.emit('rpi_car_control',"45");
		}else if(rightjoystick>=0.70 && rightjoystick<0.80){
			console.log("Reverse 46")
			socket.emit('rpi_car_control',"46");
		}else if(rightjoystick>=0.80 && rightjoystick<0.90){
			console.log("Reverse 47")
			socket.emit('rpi_car_control',"47");
		}else if(rightjoystick>=0.90 && rightjoystick<=1.00){
			console.log("Reverse 48")
			socket.emit('rpi_car_control',"48");
		}
		// Stop Left/Right
		if(leftjoystick>-0.10 && leftjoystick<0.10){
			console.log("Stop leftjoystick")
			socket.emit('rpi_car_control',"20");

		// Left
		}else if(leftjoystick<=-0.10){
			console.log("Left")
			socket.emit('rpi_car_control',"50");

		// Right
		}else if(leftjoystick>=0.10){
			console.log("Right")
			socket.emit('rpi_car_control',"60");
		}
	}
	window.requestAnimationFrame(getGamepadState)
}
...
function registerUsername() {
	if($('#username').length === 0) {
        ...
        socket.on('connect', function(){
          console.log('Connected to Car Streaming Control Server!')
        })

        socket.on('disconnect', function(){
          console.log('Disconnected from Car Streaming Control Server!')
        })

        socket.on('reconnecting', function(attemptNumber){
          console.log('Reconnecting to Car Streaming Control - Attempt: ' +attemptNumber)
        });
